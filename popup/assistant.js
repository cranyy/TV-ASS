...
function cloneFilterParamOptions() {
  const primarySelect = document.getElementById('optFilterParamName')
  const secondarySelect = document.getElementById('optFilterParamName2')
  if (!primarySelect || !secondarySelect)
    return
  const optionsHtml = primarySelect.innerHTML
...
function setClickEvents() {
  for(let elId of ['uploadSignals', 'testStrategy', 'previewStrategyTestResults', 'downloadStrategyTestResults', 'getStrategyTemplate', 'downloadStrategyTestParameters', 'uploadStrategyTestParameters', 'clearAll', 'show3DChart']) {
...
function setPopupInputsByOptions(getResults) {
  chrome.storage.local.get('iondvOptions', (getResults) => {
    ...
    if (getResults.hasOwnProperty('iondvOptions')) {
      ...
      if (iondvOptions.hasOwnProperty('optFilter2Ascending') && document.getElementById('optFilter2More') &&
        document.getElementById('optFilter2Less') && document.getElementById('optFilter2Off')) {
        if (iondvOptions.optFilter2Ascending === true) {
          document.getElementById('optFilter2Off').checked = false
          document.getElementById('optFilter2More').checked = true
          document.getElementById('optFilter2Less').checked = false
        } else if (iondvOptions.optFilter2Ascending === false) {
          document.getElementById('optFilter2Off').checked = false
          document.getElementById('optFilter2More').checked = false
          document.getElementById('optFilter2Less').checked = true
        } else {
          document.getElementById('optFilter2Off').checked = true
          document.getElementById('optFilter2More').checked = false
          document.getElementById('optFilter2Less').checked = false
        }
      }
      if (document.getElementById('optFilterValue2') && iondvOptions.hasOwnProperty('optFilterValue2'))
        document.getElementById('optFilterValue2').value = iondvOptions.optFilterValue2
      if (document.getElementById('optFilterParamName2') && iondvOptions.hasOwnProperty('optFilterParamName2'))
        document.getElementById('optFilterParamName2').value = iondvOptions.optFilterParamName2
      ...
      if(document.getElementById('gaMinTradesTotal') && iondvOptions.hasOwnProperty('gaMinTradesTotal')) {
        const val = iondvOptions.gaMinTradesTotal
        document.getElementById('gaMinTradesTotal').value = (val === null || typeof val === 'undefined') ? '' : String(val)
      }
...
function getOptions(signal) {
  ...
  iondvOptions.optFilter2Ascending  = document.getElementById('optFilter2More').checked ? true : document.getElementById('optFilter2Less').checked ? false : null
  iondvOptions.optFilterValue2 = document.getElementById('optFilterValue2').value || '50'
  iondvOptions.optFilterParamName2 = document.getElementById('optFilterParamName2').value || 'Total Closed Trades: All'
  ...
  iondvOptions.autoBestDownload = document.getElementById('autoBestDownload').checked
  ...
  if (document.getElementById('gaMinTradesTotal')) {
    const raw = document.getElementById('gaMinTradesTotal').value
    if (raw === '' || raw === null || typeof raw === 'undefined')
      iondvOptions.gaMinTradesTotal = null
    else {
      const num = parseInt(raw, 10)
      iondvOptions.gaMinTradesTotal = Number.isFinite(num) && num >= 0 ? num : null
    }
  }
