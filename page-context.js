...
// guard to prevent double-load error
if (typeof window._iondvPageContextLoaded !== 'undefined') {
  console.log('[page-context] Already loaded, skipping re-initialization')
} else {
window._iondvPageContextLoaded = true
...
window.addEventListener('message', async function (event) {
  ...
  switch (event.data.action) {
    ...
    // getChartSession bridge: reads sessionId from chart main series properties
    case 'getChartSession': {
      try {
        const cwc = window.TradingView?.bottomWidgetBar?._options?.chartWidgetCollection
        const acw = cwc?.activeChartWidget?._value || cwc?.activeChartWidget
        const outerModel = typeof acw?.model === 'function' ? acw.model() : acw?._model
        const model = outerModel?.m_model || outerModel
        const mainSeries = (typeof model?.mainSeries === 'function' ? model.mainSeries() : null) || model?._mainSeries
        const props = mainSeries?._properties
        const sessionNode = props?.sessionId
        let sessionId = null
        if (sessionNode) {
          sessionId = typeof sessionNode.value === 'function' ? sessionNode.value() : sessionNode._value
        }
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { sessionId: sessionId || null }
        }, event.origin)
      } catch (err) {
        console.warn('[TV-ASS] getChartSession error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { sessionId: null, error: err?.message || String(err) }
        }, event.origin)
      }
      break
    }
    // setChartSession bridge: writes sessionId to chart main series properties (mirrors the applyNodeValue owner-notification pattern)
    case 'setChartSession': {
      try {
        const targetSession = event.data?.data?.sessionId
        if (!targetSession) {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'No sessionId provided' }
          }, event.origin)
          break
        }
        const cwc = window.TradingView?.bottomWidgetBar?._options?.chartWidgetCollection
        ...
        if (!sessionNode) {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'sessionId property node not found' }
          }, event.origin)
          break
        }
        if (typeof sessionNode.setValue === 'function') {
          sessionNode.setValue(targetSession)
        } else if (typeof sessionNode.setValueSilently === 'function') {
          sessionNode.setValueSilently(targetSession)
          if (sessionNode._owner && typeof sessionNode._owner._childChanged === 'function')
            sessionNode._owner._childChanged(sessionNode)
        } else {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'No setter available for sessionId' }
          }, event.origin)
          break
        }
        window.postMessage({
        ...
        console.warn('[TV-ASS] setChartSession error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
    ...
    // getChartStyle bridge (read-only): returns the chart TYPE int (0 Bars / 1 Candles / 2 Line / 3 Area / 8 Heikin Ashi …) via the same main-series model path as getChartSession (props.style is the chart-type node). 0 (Bars) is valid, so the raw integer is returned and nulled only when the node is missing or the value is not a number — never `style || null`.
    case 'getChartStyle': {
      try {
        const cwc = window.TradingView?.bottomWidgetBar?._options?.chartWidgetCollection
        const acw = cwc?.activeChartWidget?._value || cwc?.activeChartWidget
        ...
        const mainSeries = (typeof model?.mainSeries === 'function' ? model.mainSeries() : null) || model?._mainSeries
        const props = mainSeries?._properties
        const styleNode = props?.style || (props && typeof props.childs === 'function' ? props.childs().style : null)
        let style = null
        if (styleNode) {
          const v = typeof styleNode.value === 'function' ? styleNode.value() : styleNode._value
          if (typeof v === 'number')
            style = v
        }
        window.postMessage({
        ...
        console.warn('[TV-ASS] getChartStyle error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { style: null, error: err?.message || String(err) }
        }, event.origin)
      }
      break
    }
    // setChartStyle bridge: mirrors setChartSession's owner-notification pattern against the style node; writes the chart TYPE int (instant, no settle). Accepts 0 (Bars), so the guard is `typeof !== 'number'`, not a falsy check.
    case 'setChartStyle': {
      try {
        const targetStyle = event.data?.data?.style
        if (typeof targetStyle !== 'number') {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'No numeric style provided' }
          }, event.origin)
          break
        }
        const cwc = window.TradingView?.bottomWidgetBar?._options?.chartWidgetCollection
        ...
        if (!styleNode) {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'style property node not found' }
          }, event.origin)
          break
        }
        if (typeof styleNode.setValue === 'function') {
          styleNode.setValue(targetStyle)
        } else if (typeof styleNode.setValueSilently === 'function') {
          styleNode.setValueSilently(targetStyle)
          if (styleNode._owner && typeof styleNode._owner._childChanged === 'function')
            styleNode._owner._childChanged(styleNode)
        } else {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { success: false, error: 'No setter available for style' }
          }, event.origin)
          break
        }
        window.postMessage({
        ...
        console.warn('[TV-ASS] setChartStyle error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { success: false, error: err?.message || String(err) }
    ...
    // getStrategyName bridge: non-mutating strategy-name lookup via getStrategyTitle(), for pre-validation without opening the dialog
    case 'getStrategyName': {
      try {
        ...
        if (!dispatcher) {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { name: null, error: 'Strategy dispatcher unavailable' }
          }, event.origin)
          break
        }
        ...
        if (!strategies.length) {
          window.postMessage({
            name: 'iondvPage', action: event.data.action,
            requestId: event.data.requestId,
            data: { name: null, error: 'No strategies on chart' }
          }, event.origin)
          break
        }
        const name = getStrategyTitle(strategies[0])
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { name: name || null }
        ...
        console.warn('[TV-ASS] getStrategyName error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { name: null, error: err?.message || String(err) }
        }, event.origin)
      }
      break
    }
    // getChartSymbol bridge (read-only): returns the exchange-qualified symbol via the same main-series model path as getChartSession, so winner CSVs capture an unambiguous tickerFull ("EXCHANGE:SYMBOL") instead of the ambiguous short legend name.
    case 'getChartSymbol': {
      try {
        const cwc = window.TradingView?.bottomWidgetBar?._options?.chartWidgetCollection
        const acw = cwc?.activeChartWidget?._value || cwc?.activeChartWidget
        ...
        let shortName = null
        let exchange = null
        // capture the canonical listing symbol (symbolInfo.pro_name, e.g. "NASDAQ:AAPL"), NOT mainSeries.symbol()/full_name which can be the data-feed id ("BATS:AAPL" = the Cboe feed) that TradingView's symbol search cannot resolve. pro_name is built from the listing exchange so it never emits a feed source id like BATS; only US-stock-on-Cboe-feed diverges. mainSeries.symbol() is kept as a last-resort fallback when pro_name and listed_exchange are both unavailable.
        try {
          const si = typeof mainSeries?.symbolInfo === 'function' ? mainSeries.symbolInfo() : mainSeries?._symbolInfo
          if (si) {
            symbol = si.pro_name || (si.listed_exchange && si.name ? `${si.listed_exchange}:${si.name}` : null) || si.full_name || null
            shortName = si.name || null
            exchange = si.exchange || null
          }
        } catch {}
        try {
          if (!symbol)
            symbol = typeof mainSeries?.symbol === 'function' ? mainSeries.symbol() : null
        } catch {}
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { symbol: symbol || null, shortName: shortName || null, exchange: exchange || null }
        }, event.origin)
      } catch (err) {
        console.warn('[TV-ASS] getChartSymbol error:', err)
        window.postMessage({
          name: 'iondvPage', action: event.data.action,
          requestId: event.data.requestId,
          data: { symbol: null, shortName: null, exchange: null, error: err?.message || String(err) }
        }, event.origin)
      }
      break
    }
    default:
      ...
      window.postMessage({name: 'iondvPage', action: event.data.action, data: null, message: `Unknown action: ${event.data.action}`}, event.origin)
  }
})
...
async function setStrategyParamsFromPage(request) {
  ...
  // only call updateAllViews when params were actually updated, passing the data-source-change object for a proper UI refresh — a zero-update call fires data-source-change that puts TV in a transitional state, blocking the legacy setter from opening the strategy dialog
  if (result.updated.length > 0 && typeof strategy.updateAllViews === 'function') {
    try {
