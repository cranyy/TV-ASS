...
storage.generateChunkKey = (type = 'data') => {
  return `${storage.CHUNK_PREFIX}_${type}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
}

storage.saveChunk = async (chunkKey, chunkData) => {
  if (!chunkKey)
    return
  const chromeKey = `${storage.KEY_PREFIX}_${chunkKey}`
  return new Promise(resolve => {
    chrome.storage.local.set({ [chromeKey]: chunkData }, () => resolve())
  })
}

storage.loadChunks = async (chunkKeys = []) => {
  if (!chunkKeys || !chunkKeys.length)
    return []
  const chromeKeys = chunkKeys.map(key => `${storage.KEY_PREFIX}_${key}`)
  return new Promise(resolve => {
    chrome.storage.local.get(chromeKeys, (results) => {
      const data = chunkKeys.map((key, idx) => results[chromeKeys[idx]] || [])
      resolve(data)
    })
  })
}

storage.removeChunks = async (chunkKeys = []) => {
  if (!chunkKeys || !chunkKeys.length)
    return
  const chromeKeys = chunkKeys.map(key => `${storage.KEY_PREFIX}_${key}`)
  return new Promise(resolve => {
