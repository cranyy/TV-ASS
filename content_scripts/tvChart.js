...
tvChart.getCurrentTimeFrame = async () => {
  // parse timeframe from aria-label/data-tooltip instead of innerText (Jan 2026 TV UI change)
  ...
  // Try aria-label (e.g. "3 minutes"), then data-tooltip, then innerText as fallback
  let curTimeFrameText = curTimeFrameEl.getAttribute('aria-label') ||
                          curTimeFrameEl.getAttribute('data-tooltip') ||
                          curTimeFrameEl.innerText

  if (!curTimeFrameText) {
    throw new Error('Could not extract timeframe text from element')
  }

  // Convert "X minutes" format to "Xm" format (e.g. "3 minutes" → "3m")
  curTimeFrameText = curTimeFrameText.replace(/\s+minutes?/i, 'm').replace(/\s+hours?/i, 'h').replace(/\s+days?/i, 'd').replace(/\s+weeks?/i, 'w').replace(/\s+months?/i, 'M')
